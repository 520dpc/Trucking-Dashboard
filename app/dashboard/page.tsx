...
function sum(values: (number | null | undefined)[]): number { // Declares a sum helper that accepts an array of possibly-null numbers.
  return values.reduce((acc, v) => acc + (v ?? 0), 0); // Reduces the array, treating null/undefined as 0 and returning the total.
}

// Helper to compute percent change vs a previous value.
function percentChange(current: number, previous: number): number | null { // Declares a helper to compute percentage change.
  if (previous <= 0) return null; // If previous is zero or negative, returns null to avoid divide-by-zero and nonsense.
  return ((current - previous) / previous) * 100; // Computes ((current - previous) / previous) * 100 to get percent change.
}

// Helper to approximate utilization based on miles vs a theoretical maximum.
function approxUtilization(totalMiles: number, days: number): number { // Declares a helper that computes approximate utilization.
  const maxMilesPerDay = 600; // Assumes ~600 miles/day per truck as a theoretical upper bound.
  const theoreticalMax = maxMilesPerDay * days; // Computes maximum possible miles over the period.
  if (theoreticalMax <= 0) return 0; // If no days or invalid input, returns 0% utilization.
  const ratio = (totalMiles / theoreticalMax) * 100; // Computes actual miles as a percentage of theoretical max.
  return Math.max(0, Math.min(100, ratio)); // Clamps the result between 0 and 100 to keep it in a valid range.
}

// Helper to compute whole days between two Date objects.
function daysBetween(later: Date, earlier: Date): number { // Declares a helper to compute day difference between two dates.
  const msPerDay = 1000 * 60 * 60 * 24; // Number of milliseconds in one day.
  const diffMs = later.getTime() - earlier.getTime(); // Milliseconds difference between the two dates.
  return Math.floor(diffMs / msPerDay); // Converts the difference to whole days by flooring the division.
}
...
export default async function DashboardPage() { // Default export: async React Server Component for the dashboard route.
  ...
  const staleThresholdDays = 14; // Defines how many days of silence counts as "stale" for customers (reduced from 30 to 14).
  ...
  const expensesDeltaPercent = percentChange(
    totalExpensesCurrent + totalFuelCurrent, // Uses total operating expenses (general + fuel) for current.
    totalExpensesPrev + totalFuelPrev // Uses total operating expenses for previous period.
  );
  ...
  const staleCustomers = customersWithNotes.filter((customer) => {   // Filters customers that are considered "stale".
    ...
    if (!lastNote) {                                                 // If there has never been a call note for this customer...
      return true;                                                   // ...treat them as stale automatically (never contacted).
    }

    const lastContact = lastNote.createdAt;                          // For customers with notes, use the last note date.
  ...
  const topStaleCustomers = staleCustomers // Prepares a limited list of stale customers for the widget.
    .slice(0, 5) // Takes only the first 5 to keep the card readable.
    .map((customer) => { // Maps each customer into the shape expected by the stale customers widget.
      const lastNote = customer.callNotes[0]; // Gets the most recent note again (if any).
      const lastContact = lastNote?.createdAt ?? customer.createdAt; // Uses note date when present, otherwise creation date.
      const daysStale = daysBetween(now, lastContact); // Computes how many days since last contact or since creation.
      const neverContacted = !lastNote; // Flags customers with no notes at all as "never contacted".

      return {
        id: customer.id, // Unique id of the customer for React keys.
        name: customer.name ?? "Unknown", // Customer display name or fallback.
        daysStale, // Number of days stale.
        neverContacted, // Whether this customer has never had a call note.
      };
    });
    ...
    <div className="space-y-6 p-4"> {/* Outer wrapper with padding and vertical spacing between sections. */}
      ...
      <DashboardGridClient initialLayout={initialLayout}> {/* Renders the grid of widgets using the configured layout. */}
        ...
        <StaleCustomersWidget
          ...
          thresholdDays={staleThresholdDays} // Passes the day threshold for staleness (14 days).
          customers={topStaleCustomers} // Passes the top stale customers to render as a list in the card.
        ...
        <UtilizationWidget
          ...
          periodLabel={periodLabel} // Passes the period label.
          deltaPercent={null} // Not yet computing utilization change vs prior period, so passes null.
